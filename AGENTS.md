...
**Not started:** the Type B reverse drill (build the solid from three views) — designed and specced, not planned or built.

---

## 4. Next up

**START HERE (updated 2026-08-29).**

**State (2026-08-29):** eleven exercises across two topics, a designed UI, 390 tests green. Everything is merged — PR #16 (UI revamp) and PR #17 (the stranded handoff commit) both landed, and the merged local branches were pruned. `main` is clean and the working tree is clean.

**The one unmet criterion is still a student.** Spec §10 asks for a GMI student completing a drill cold and agreeing the feedback is correct. The builder has done it; he knows what the app expects, which is the bias that test exists to defeat. It needs no deploy — sit beside someone with `npm run dev` running and watch where they hesitate, not just what they score.

### Next two pieces of work, agreed 2026-08-27

**1. Oblique projection — a new Tier 1 topic.**

Cheapest real topic left: it reuses the solid model entirely, so it is a projection variant rather than new geometry.

**The lattice check is DONE — run 2026-08-29, results in §1.1 and reasoning in `docs/decision-log.md`. Do not re-run it; build on it.** Two decisions came out of it and both are settled:

- **All three types ship, as one generator parameterised by the depth factor `k`.** Oblique projects `(x,y,z) -> (x + k·y, z + k·y)`; cavalier is `k = 1`, cabinet `k = 1/2`, general `k = 2/3`. Each is exact — 27104 of 27104 — provided **every y-coordinate in the solid is a multiple of `k`'s denominator**: step 1 for cavalier, 2 for cabinet, 3 for general. That is the base depth *and* every feature box's `y` and `d`, not just the overall depth, and it belongs in validation rather than in authoring care. Author at least one solid in all three, because the comparison IS the teaching.
- **The receding angle is fixed at 45°.** Only `tan 45° = 1` gives an integer step; 15°, 30°, 60° and 75° are all irrational. It is the standard angle anyway, but it is not something the topic can teach by varying.
- **The depth is one grid diagonal per unit — which is `√2` longer than metrically true scale, and the hints must say so.** Strict "full size" on a 45° axis needs an offset of `k·y/√2`, irrational in 0 of 80 tries. The *ratios* between the three types are exact, and those are the teaching content; the `√2` is uniform and invisible without a ruler, which the app does not have. **But a hint reading "depth is drawn full size" would contradict the key** — the parabola-hint failure class (§6). Say "one grid diagonal per unit of depth", and cite the convention per §7 before authoring.
- **Box-only solids. No cylinders.** This is the finding the prediction missed, and it is independent of `k` — no choice of type rescues it. A bore on x or z projects to an ellipse (2.618:1) and `circle` is our only curved primitive; a bore on y keeps a circular rim but its silhouette tangents land at `c ± r/√2`, irrational in 450 of 450 tries. **A through-hole cannot be drawn in oblique on this grid at all.** It is Tier 2 now, filed with tangents and ellipses.

The restriction is affordable: 5 of the 8 orthographic solids are already box-only and reusable as oblique subjects, so the topic has content without inventing any. `validateSolid`'s oblique path must REJECT a solid carrying a cylinder rather than projecting one badly.

What remains is the ordinary work: the four things §7 requires — generator, hints, method diagram, preview figure.

**2. A settings page, with canvas paper themes.**

Requested by the builder 2026-08-27. Preferences live per-viewer in `localStorage` — v1 has no accounts and none are wanted (§3 of the design spec). Three paper options:

- **White** — today's behaviour, and the default. A technical drawing is black on white.
- **Warm/yellowish paper** — easier on the eyes for a long session.
- **Dark paper with inverted lines** — the builder's words: "for weirdos". Ink goes light on a dark sheet.

**THE TRAP, AND IT IS A REAL ONE.** `Pictorial.tsx` hard-codes `#ffffff` for both its ground and its face fills, and that fill colour must equal the ground EXACTLY — later fills overpainting earlier strokes is the whole hidden-line mechanism (§6). The moment paper becomes a user preference:

- `Pictorial` must read the paper token rather than a literal, and its fills must track it exactly. A mismatch of one hex digit shows as hidden edges reappearing, or the drawing rendering as a white silhouette.
- `--paper`, `--grid`, `--ink` and `--centre` are currently fixed in both themes on purpose. That rule is being deliberately relaxed into a user choice — update the comment in `globals.css` rather than leaving it contradicting the code.
- The dark-paper option inverts `--ink`, so anything that assumes dark ink (the feedback overlay's tones, `--centre` red, `--construction`) needs checking against it for contrast.
- **Verify by rendering all three, on both an orthographic and a parabola exercise.** This is the exact class of bug that only a screenshot catches.

### Then, in rough order of value

- **Type B, the reverse drill** — the biggest outstanding promise, specced in `2026-08-26-canvas-and-reverse-drill-design.md` §4, never planned. Its riskiest piece is generalising `isoproject.ts` to four viewpoints: a wrong sign there yields a picture that is perfectly self-consistent and perfectly mirrored, the same failure class the golden set exists to catch, so it needs the same treatment.
- **Tier 2 scoring** (tangents, ellipses) — needs a design conversation before any code. "Close enough" deserves the scrutiny §5.2 gave the generator.
...
- **`gh` needs the account named now:** `GH_TOKEN=$(gh auth token --user adamafzainizam) gh pr create --fill`. The old `GITHUB_TOKEN=` trick stopped working when a second keyring account appeared (§6).
- **Subagents have been hitting rate limits.** Several recent chunks were done directly in the main session. Plan for work spanning sessions, and commit per area so an interruption loses little.
