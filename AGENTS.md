...
**START HERE (updated 2026-08-27, night — written as a handoff).**

**State:** eleven exercises across two topics, a designed UI, 390 tests green. PR #16 (the UI revamp) may still be open — check before branching. Everything before it is merged.
...
**Run the lattice check FIRST (§1.1), before writing anything.** The expectation is that cavalier oblique IS lattice-exact when the receding axis steps one grid unit diagonally per unit of depth — which is exactly how it is drawn on squared paper — but that is a prediction, not a measurement, and this project does not build on predictions. Cabinet oblique (half depth) is only lattice-exact for even depths; decide deliberately which to ship.

It owes the four things §7 requires: generator, hints, method diagram, preview figure.
