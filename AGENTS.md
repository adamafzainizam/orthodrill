...
| Cavalier oblique, box geometry | **19683 of 19683** cell corners |
| Cabinet oblique, odd depths | **0 of 294** — even depths are all 294 of 294 |
...
**Tier 1 — buildable now, lattice-exact.** Orthographic projection (shipped, 8 exercises), parabola by the rectangle method (shipped, 3), **cavalier oblique on box-only solids** (measured 2026-08-29), the Type B reverse drill, and straightedge constructions such as bisectors and equal divisions. Each needs a generator, hints, and a method diagram or pictorial — nothing else.
...
- **Cavalier, not cabinet.** Cavalier (`(x,y,z) -> (x+y, z+y)`) is exact for all box geometry — 19683 of 19683 cell corners. Cabinet's half-depth is exact only for even depths (0 of 294 odd), which would silently constrain every exercise's depth and make an odd-depth solid unscoreable rather than merely wrong.
- **Box-only solids. No cylinders.** This is the finding the prediction missed. A bore on x or z projects to an ellipse (2.618:1) and `circle` is our only curved primitive; a bore on y keeps a circular rim but its silhouette tangents land at `c ± r/√2`, irrational in 450 of 450 tries. **A through-hole cannot be drawn in oblique on this grid at all.** It is Tier 2 now, filed with tangents and ellipses.
