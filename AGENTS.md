...
**START HERE (updated 2026-08-27, evening).** **The drill works. A person drew three views and got a correct perfect score.** What it now needs is other people, and more content — not more machinery.

Three things, in the order they are worth doing:

1. **Put it in front of a student who has never seen it.** Spec §10's actual test is a GMI student completing a drill cold and agreeing the feedback is correct and useful. The builder passing is necessary and not sufficient — he knows what the app expects, which is exactly the bias the test exists to defeat. Watch where they hesitate rather than only what they score.
2. **Then Type B**, the reverse drill — designed and specced in `docs/superpowers/specs/2026-08-26-canvas-and-reverse-drill-design.md` §4, not yet planned.

**Drill content reached the spec's 8-12 range on 2026-08-28** (11 exercises: 8 orthographic, 3 parabola — see §3). What's still open, for whoever picks this up next:

- The four new orthographic solids (`hidden-groove`, `near-mirror-notches`, `bore-along-length`, `step-and-notch`, in `src/drills/registry.ts`) are unverified against any external source, unlike the four golden-set fixtures. They lean entirely on the generator's own 2026-08-26 sign-off rather than a citation or a second reviewer. Each pictorial was rendered and read as a student before shipping (screenshots, not just tests) and the prompts were each checked against "would a student who followed this exactly produce the key?", but that is authoring self-review, not the golden set's external check. Worth a second pass if the drill is ever reported wrong.
- `parabola-rectangle-4` and `parabola-rectangle-6` reuse the same `parabolaKey`/`parabolaBounds` functions the seeded `n=5` exercise already exercises in `parabola.test.ts`, so they carry no new risk in the generator itself — only in placement, which `parabola.test.ts`'s `SHIP_NS = [3, 4, 5, 6]` already covered before this session started.
