...
**START HERE (added 2026-08-26).** Two things, in this order:

1. **Get the golden set verified — it is the oldest outstanding item and it blocks shipping any drill.** Run `npm run verify:sheet`, which writes `verification-sheet.html` (gitignored, ~119KB, self-contained, inline SVG, no external assets). It now shows each part's isometric pictorial beside its three orthographic views, so a reviewer can check the picture against the drawings instead of reading coordinates. **The intended route is to publish it as a private Artifact and send the builder that link** to pass to a GMI contact who knows drafting — publishing requires reading the whole generated file first, which is cheap at the start of a session and was skipped at the end of the last one. Alternatives if that is unavailable: email the file, or screenshot it. All four parts are still marked UNVERIFIED and nothing has been checked by a human who knows the subject.
